...
public class LinkedBlockingQueueExWithProduceConsumer {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		/*
		 * LinkedBlockingQueue is an optionally bounded, thread-safe implementation of the BlockingQueue interface backed by linked nodes.
		 * It is part of java.util.concurrent and is commonly used in producer-consumer scenarios.
		 * The queue can be created with a specified capacity (bounded) or with no capacity limit (unbounded, limited only by memory).
		 * Elements are ordered in FIFO (First-In-First-Out) manner.
		 *
		 * Common LinkedBlockingQueue Methods:
		 *  - add(item):         Inserts the specified element if space is available (throws exception if full, only relevant if bounded)
		 ...
		 *
		 * Notes:
		 *  - The capacity of the queue can be set at creation; if not set, the queue is unbounded.
		 ...
		 *  - LinkedBlockingQueue typically has higher throughput than ArrayBlockingQueue due to less frequent locking.
		 *
		 */
		
		// Example scenario: Similar to Uber, Rapido, or Ola – simulating booking requests and available drivers.
		//1. Create a new linked blocking queue
